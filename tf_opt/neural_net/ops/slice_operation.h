...
#ifndef LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_SLICE_OPERATION_H_
#define LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_SLICE_OPERATION_H_
...
class SliceOperation : public Operation {
 public:
  ...
  static MaybeForGraph<SliceOperation> CreateForGraph(std::string op_name,
  ...
  const std::vector<int64>& begin() const { return begin_; }
  const std::vector<int64>& sizes() const { return sizes_; }
 ...
 private:
  SliceOperation(std::string op_name, Shape input_shape, Shape output_shape,
                 std::vector<int64> begin, std::vector<int64> sizes)
...
#endif  // LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_SLICE_OPERATION_H_
