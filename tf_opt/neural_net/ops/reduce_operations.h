...
// scalar.
#ifndef LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_REDUCE_OPERATIONS_H_
#define LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_REDUCE_OPERATIONS_H_
...
#include "learning/brain/research/tf_opt/shared/graph/operation_validator.h"
...
class LinearReduceOperation : public Operation {
 public:
  static absl::StatusOr<LinearReduceOperation<R>> Create(
      std::string op_name, Shape input_shape, const std::vector<int64>& axes);
  ...
  const std::vector<int64>& axes() const { return axes_; }
 ...
 private:
  LinearReduceOperation(std::string op_name, Shape input_shape,
                        Shape output_shape, const std::vector<int64>& axes)
...
proto::TensorNode LinearReduceOperation<R>::ToProto(
  ...
  *result.mutable_out_dimension() = output_shape().AsProto();
  ...
  axes->set_name(reduce::kOptionsAxesKey);
...
NonlinearReduceOperation<R>::CreateForGraph(
    std::string op_name, const Operation* input, const std::vector<int64>& axes,
    MaximumImplementationType formulation) {
  return FromMaybeCreated(
...
proto::TensorNode NonlinearReduceOperation<R>::ToProto(
  ...
  *axes->mutable_value() = {axes_.begin(), axes_.end()};
  if (formulation_ != MaximumImplementationType::kOptimalBigM) {
    ...
    formulation->set_name(reduce::kOptionsFormulationKey);
...
#endif  // LEARNING_BRAIN_RESEARCH_TF_OPT_SHARED_OPS_REDUCE_OPERATIONS_H_
