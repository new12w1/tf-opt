...
#include "learning/brain/research/tf_opt/shared/graph/operation_testing.h"
...
TEST(SliceOperationTest, SimpleCreate) {
  ...
  const std::vector<int64> begin({1, 1});
  const std::vector<int64> sizes({2, 2});
...
TEST(SliceOperationTest, SimpleCreateInvalidSize) {
  const Shape input_shape({3, 3});
  const std::vector<int64> begin({1, 1});
  const std::vector<int64> sizes({4, 2});
...
Operation::Options MakeOptions(const std::vector<int64>& begin,
