...
absl::StatusOr<SliceOperation> SliceOperation::Create(
    std::string op_name, Shape input_shape, std::vector<int64> begin,
...
MaybeForGraph<SliceOperation> SliceOperation::CreateForGraph(
    std::string op_name, const Operation* input, std::vector<int64> begin,
    std::vector<int64> sizes) {
...
absl::StatusOr<SliceOperation> SliceOperation::GenericCreate(
  ...
  TFOPT_ASSIGN_OR_RETURN(std::vector<int64> size,
